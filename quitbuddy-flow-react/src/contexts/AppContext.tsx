...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const savedState = localStorage.getItem('quitqly-app-state');
    if (savedState) {
      try {
        const parsed = JSON.parse(savedState);
        if (parsed.userCoins) setUserCoinsState(parsed.userCoins);
        if (parsed.selectedCharacter) setSelectedCharacterState(parsed.selectedCharacter);
        if (parsed.selectedBackground) setSelectedBackgroundState(parsed.selectedBackground);
        if (parsed.ownedCharacters) setOwnedCharacters(parsed.ownedCharacters);
        if (parsed.ownedBackgrounds) setOwnedBackgrounds(parsed.ownedBackgrounds);
        if (parsed.ownedAccessories) setOwnedAccessories(parsed.ownedAccessories);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('quitqly-app-state', JSON.stringify(stateToSave));
