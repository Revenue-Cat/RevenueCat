  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="px-6 py-8 max-w-md mx-auto">
        ...
        >
          ...
          <div className="grid grid-cols-4 gap-3">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="aspect-square bg-surface-light rounded-xl"
              ></div>
            ))}
          </div>
        </Card>
        ...
        <Card
          className="p-4 mb-6 cursor-pointer"
          onClick={() => openShopWithTab("characters")}
        >
          <h3 className="font-semibold mb-3 flex items-center justify-between">
            Characters
            <ChevronRight className="w-5 h-5 text-muted-foreground" />
        ...
        <Card
          className="p-4 mb-6 cursor-pointer"
          onClick={() => openShopWithTab("backgrounds")}
        >
          <h3 className="font-semibold mb-3 flex items-center justify-between">
            Backgrounds
            <ChevronRight className="w-5 h-5 text-muted-foreground" />
          ...
          <div className="grid grid-cols-4 gap-3">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="aspect-square bg-gradient-to-br from-blue-100 to-purple-100 rounded-xl"
              ></div>
            ))}
          </div>
        </Card>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 bg-primary/10 rounded-lg border-2 border-primary">
              <div>
                ...
                <div className="text-s text-muted-foreground">Save 80%</div>
              </div>
            ...
            <div className="space-y-4 mb-6">
              {currentField.options.map((option) => (
                <button
                  ...
                  onClick={() =>
                    handleSelection(
                      editingField as keyof SmokingHabits,
                      option.value
                    )
                  }
                  className={`w-full p-4 rounded-2xl flex items-center justify-between transition-colors ${
                    smokingHabits[editingField as keyof SmokingHabits] ===
                    option.value
